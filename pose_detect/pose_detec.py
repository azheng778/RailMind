...
def find_vertical_slope_and_angle(preds, intersection, horizontal_slope, center_2_3, output_frame, class_id=1,
    ...
    for det in preds:
        if int(det[5]) == class_id:
            ...
            try:
                ...
                results.append((m_vertical, np.abs(angle_deg * 90 / np.abs(angle_with_horizontal))))
...
def warp_and_crop_roi(image, preds, class_id=0, output_size=500, margin=100):
    ...
    for det in preds:
        ...
        valid_kpts = kpts[kpts[:, 2] > 0.1]  # 筛选有效关键点（conf>0.1）
...
class YOLOv8Keypoint:
    ...
    def __init__(self,
                 det_ckpt_path="./model_pose_detec/best.onnx",
    ...
    def predict(self, image, visualize=False, camera_id=0, threshold=200):
        #定义水平斜率
        ...
        # 推理图片
        output_frame, preds = self.process_image(np.copy(image))
        ...
        if result:
            ...
            if cv_horizontal_slope is not None:
                horizontal_slope = cv_horizontal_slope

            # info.append(f"交点坐标: {intersection}")
            # info.append(f"水平斜率: {horizontal_slope}")

            # 调用函数计算垂直斜率和偏差角
            vertical_results = find_vertical_slope_and_angle(class_1_preds, intersection, horizontal_slope, center_2_3, output_frame, class_id=1)
            if vertical_results:
                for i, (vertical_slope, angle_deg) in enumerate(vertical_results):
                    ...
                    if angle_deg <= 30:
                        ...
                        return_info.append(
                            ['F', f'把手{i}警告', int(bbox[0]), int(bbox[1]), int(bbox[2] - bbox[0]), int(bbox[3] - bbox[1])])
                    else:
                        level = 2
                        text_color = (0, 0, 255)
                        return_info.append(
                            ['F', f'把手{i}异常', int(bbox[0]), int(bbox[1]), int(bbox[2] - bbox[0]), int(bbox[3] - bbox[1])])
            ...
            if len(class_1_preds) and board_error == False:
                return_info.append(
        ...
        if visualize:
            return return_info, output_frame

        # 直接返回 return_info 列表
        return return_info

# 实例化螺栓松动检测
handle_track = YOLOv8Keypoint()


# c++与摄像头参数传递接口
def predict(frame, id=0):
    result = handle_track.predict(frame)
    print("the code is running here ------------------",result)
    return result


# def main():
#     # 模型传入参数  onnx_model_name, confidence_thres, nms_thres
#     model = YOLOv8Keypoint()
#     frame_id = 0
#     # cap = cv2.VideoCapture('./test_data/CH8_00000100936000000.mp4')
#     cap = cv2.VideoCapture('./test_data/CH8_00000101209000000.mp4')
#     width = cap.get(cv2.CAP_PROP_FRAME_WIDTH)  # float
#     height = cap.get(cv2.CAP_PROP_FRAME_HEIGHT)  # float
#     vid_writer = cv2.VideoWriter(
#         './test_result.mp4', cv2.VideoWriter_fourcc(*"mp4v"), 1, (int(width), int(height))
#     )
#     while True:
#         frame_id += 1
...
#             continue
#         if ret_val:
#             results, vis_result = model.predict(frame, True, frame_id)
#             for each in results:
#                 print(each)
#             vid_writer.write(vis_result)
#         else:
#             break


if __name__ == "__main__":
    onnx_model_name = "./model_pose_detec/best.onnx"
    img_path = "./frame_00993.jpg"
    confidence_thres = 0.3
    nms_thres = 0.5
    #
    # # 读取图像
    ...
    #
    # # 模型加载与推理
    model = YOLOv8Keypoint(onnx_model_name, confidence_thres, nms_thres)
    result_info = model.predict(frame)
    #
    # # result_info格式:
    # # result_info[0]为相机编号
    # # result_info[1]中任意一个元素中索引为0时，后两位为检测的把手的角度及风险程度，角度代表与垂直方向的偏差角，风险程度分为0、1、2三个程度，分别表示0-30度，30-60度，60度以上
    # # result_info[1]中任意一个元素中索引为1时，后两位表示对盖子中心区域进行边缘检测的边缘线像素值与警告信息，0为正常即较平坦，1表示有线条即区域不是平坦的盖子
    #
    print(result_info)  # 打印返回的信息
    #
    # # 打印信息解析样例
    # print(f"相机编号为: {result_info[0]}")
    # info = result_info[1]
    # if len(info) > 0:
    #     handle_num = 0
    #     for result in info:
    #         if result[0] == 0:
    #             print(f"把手角度为：{result[1]}, 风险等级为{result[2]}")
    #             handle_num += 1
    #         else:
    #             if result[2] == 0:
    #                 print(f"板子正常，canny边缘检测到的杂乱边缘点有{result[1]}个")
    #             elif result[2] == 1:
    #                 print(f"板子异常，canny边缘检测到的杂乱边缘点有{result[1]}个")
    #     if handle_num != 2:
    #         print("未检测到两个把手，警告！")
    # else:
    #     print("未检测到任何信息，警告！")
    ...
    #     cv2.resizeWindow("Output", output_frame.shape[1], output_frame.shape[0])

...
class YOLOv8Keypoint:
    ...
    def __init__(self,
                 det_ckpt_path="./model_pose_detect/best.onnx",
...
#
#     # 指定包含视频文件的根目录
#     root_dir = "F:\cumt-yolo\data/250815err_video"
...
#             for video_path in glob.glob(os.path.join(root_dir, '**', extension), recursive=True):
#                 f.write(f"Processing video: {video_path}\n")
#                 print(f"Processing video: {video_path}")
#
#                 cap = cv2.VideoCapture(video_path)
#                 if not cap.isOpened():
#                     print(f"无法打开视频文件: {video_path}")
#                     f.write(f"Error: Cannot open video file {video_path}\n\n")
#                     continue
#
...
#
#                 # 为每个视频创建输出文件
#                 # output_video_name = os.path.splitext(os.path.basename(video_path))[0] + '_result.mp4'
#                 # output_video_path = os.path.join(os.path.dirname(video_path), output_video_name)
#
#                 # vid_writer = cv2.VideoWriter(
#                 #     output_video_path, cv2.VideoWriter_fourcc(*"mp4v"), 1, (int(width), int(height))
#                 # )
#
#                 frame_count = 0
#                 while True:
...
#                 # vid_writer.release()
#                 f.write("\n")
#                 print(f"完成处理视频: {video_path}\n")
#
#
# if __name__ == "__main__":
#     main()
    # onnx_model_name = "runs/train/best.onnx"
    # img_path = "dataset/YOLODataset/images/train/20250317135532.jpg"
    ...
    # #
    # # # 模型加载与推理
    # model = YOLOv8Keypoint(det_ckpt_path=onnx_model_name, confidence_thres=confidence_thres, nms_thres=nms_thres)
